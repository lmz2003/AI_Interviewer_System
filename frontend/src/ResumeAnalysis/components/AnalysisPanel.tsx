...
type TabId = 'overview' | 'keywords' | 'content' | 'jobMatch' | 'competency' | 'report';
...
const AnalysisPanel: React.FC<AnalysisPanelProps> = ({ analysis, parsedData }) => {
  ...
  const renderContent = () => (
    <>
      <Section>
        <SectionTitle icon={<FileTextIcon />} label="内容分析" />
        <div style={{ padding: '12px', background: C.bg, borderRadius: C.radiusSm, fontSize: '0.875rem', color: C.text }}>
      ...
      {contentData.sections && Object.keys(contentData.sections).length > 0 && (
        <Section>
          <SectionTitle icon={<ClipboardIcon />} label="各部分内容" />
          <ListBlock items={Object.entries(contentData.sections).map(([sec, count]) => `${sec}：${count}`)} />
        </Section>
      )}

      {parsedData && (
        <Section>
          <SectionTitle icon={<FileTextIcon />} label="解析数据" />
          <pre style={{ fontSize: '0.78rem', color: C.textMuted, background: C.bg, padding: '12px', borderRadius: C.radiusSm, overflow: 'auto', margin: 0, lineHeight: 1.6 }}>
